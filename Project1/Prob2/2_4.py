...
def astar(grid):
    if is_solvable(grid)==False:
        ...
        queue = PriorityQueue()
        queue.put((heuristic(grid), grid, ''))
        visited = set([grid])
    
        while not queue.empty():
            _, curr_grid, path = queue.get()
            ...
            for adj_pos in get_adjacent_positions(curr_grid, blank_pos):
                ...
                if new_grid_str not in visited:
                    visited.add(new_grid_str)
                    if adj_pos - blank_pos==1:
                        queue.put((heuristic(new_grid_str) + len(path)+1, new_grid_str, path +'r'))
                    elif adj_pos - blank_pos==-1:
                        queue.put((heuristic(new_grid_str) + len(path)+1, new_grid_str, path +'l'))
                    elif adj_pos - blank_pos==-3:
                        queue.put((heuristic(new_grid_str) + len(path)+1, new_grid_str, path +'u'))
                    elif adj_pos - blank_pos==3:
                        queue.put((heuristic(new_grid_str) + len(path)+1, new_grid_str, path +'d'))                   
